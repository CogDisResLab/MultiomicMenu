...
# Enrichment tools require gene symbols, so collapse only for enrichment while
# leaving the site-resolved network ranking in `combined` unchanged.
pathway_combined <- tibble::tibble(name = names(combined), rank = as.numeric(combined)) %>%
  mutate(name = phosphosite_parent(name)) %>%
  group_by(name) %>%
  summarise(rank = max(rank, na.rm = TRUE), .groups = "drop") %>%
  arrange(desc(rank)) %>%
  deframe()
...
if(rlang::has_name(data, "rna")) {
  ...
  gsea_pathways <-
    fgsea::fgsea(gmt_pathways, pathway_combined, scoreType = "pos") %>%
...
if(rlang::has_name(data, "rna")) { #If we do GSEA on the whole network, do Enrichr on the top 10% of the network, else do Enrichr on the whole network
  topcombined <- pathway_combined[pathway_combined >= quantile(pathway_combined, 0.9)]
} else {
  topcombined <- pathway_combined[pathway_combined >= 0]
}
